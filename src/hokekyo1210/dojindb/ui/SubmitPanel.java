...
public class SubmitPanel extends JPanel implements ActionListener, MouseListener,Runnable{
	...
	private void reloadTag(){
		...
		int x = 8,y = 0,count = 0;
		for(TagLabel tag:tags){
			if(((int)Main.DIAMETER*(offsetX+x*TagLabel.width+count*6+tag.getLength()*TagLabel.width)) > ((int)Main.DIAMETER*panelWidth-4)){
				///はみだすので改行
				x = 0;y++;count = 0;
				if(y == 2)break;///３行目は無いので追加を諦める
			}
			tag.reloadBounds((int)Main.DIAMETER*(offsetX+x*TagLabel.width+count*6), (int)Main.DIAMETER*(offsetY+y*28));
			this.add(tag);
			x+=tag.getLength();
			count++;
		}
